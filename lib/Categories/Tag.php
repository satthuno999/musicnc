...
namespace OCA\musicnc\Categories;

use OCA\musicnc\Service\TagManager;
...
class Tag
{
    ...
    public function getTracksDetails($tagId)
    {
        ...
        $sql->select('AT.id')
            ...
            ->from('musicnc_tracks', 'AT')
            ->leftJoin('AT', 'musicnc_artists', 'AA', $sql->expr()->eq('AT.artist_id', 'AA.id'))
            ->leftJoin('AT', 'musicnc_albums', 'AB', $sql->expr()->eq('AT.album_id', 'AB.id'))
    ...
    public function getCategoryItemCovers($tagId)
    {
        ...
        $sql->select('AB.id')
            ...
            ->from('musicnc_tracks', 'AT')
            ->leftJoin('AT', 'musicnc_albums', 'AB', $sql->expr()->eq('AT.album_id', 'AB.id'))
            ->leftJoin('AB', 'musicnc_artists', 'AA', $sql->expr()->eq('AB.artist_id', 'AA.id'))
            ->where($sql->expr()->in('AT.file_id', $sql->createNamedParameter($allFiles, IQueryBuilder::PARAM_INT_ARRAY)))
            ->andWhere($sql->expr()->eq('AT.user_id', $sql->createNamedParameter($this->userId)))
            ->orderBy($sql->createNamedParameter(mb_strtolower('AB.name')), 'ASC')
            ->groupBy('AB.id')
