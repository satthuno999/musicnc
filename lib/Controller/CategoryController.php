...
use OCA\musicnc\Categories\Tag;
...
class CategoryController extends Controller
{
    ...
    public function getCategoryItems($category)
    {
        ...
        if ($category === 'Artist') {
            ...
            $SQL = 'SELECT  `AB`.`id` , `AB`.`name`, LOWER(`AB`.`name`) AS `lower`
						FROM `*PREFIX*musicnc_albums` `AB`
						LEFT JOIN `*PREFIX*musicnc_artists` `AA` 
    ...
    {
        $SQL = array();
        $SQL['Artist'] = 'SELECT COUNT(`AT`.`id`) AS `count` FROM `*PREFIX*musicnc_tracks` `AT` LEFT JOIN `*PREFIX*musicnc_artists` `AA` ON `AT`.`artist_id` = `AA`.`id` WHERE  `AT`.`artist_id` = ? AND `AT`.`user_id` = ?';
        $SQL['Genre'] = 'SELECT COUNT(`AT`.`id`) AS `count` FROM `*PREFIX*musicnc_tracks` `AT` WHERE  `AT`.`genre_id` = ? AND `AT`.`user_id` = ?';
        $SQL['Year'] = 'SELECT COUNT(`AT`.`id`) AS `count` FROM `*PREFIX*musicnc_tracks` `AT` WHERE  `AT`.`year` = ? AND `AT`.`user_id` = ?';
        $SQL['Title'] = 'SELECT COUNT(`AT`.`id`) AS `count` FROM `*PREFIX*musicnc_tracks` `AT` WHERE  `AT`.`id` > ? AND `AT`.`user_id` = ?';
        $SQL['Playlist'] = 'SELECT COUNT(`AP`.`track_id`) AS `count` FROM `*PREFIX*musicnc_playlist_tracks` `AP` WHERE  `AP`.`playlist_id` = ?';
        $SQL['Folder'] = 'SELECT COUNT(`AT`.`id`) AS `count` FROM `*PREFIX*musicnc_tracks` `AT` WHERE  `AT`.`folder_id` = ? AND `AT`.`user_id` = ?';
        $SQL['Album'] = 'SELECT COUNT(`AT`.`id`) AS `count` FROM `*PREFIX*musicnc_tracks` `AT` WHERE  `AT`.`album_id` = ? AND `AT`.`user_id` = ?';
        $SQL['Album Artist'] = 'SELECT COUNT(`AT`.`id`) AS `count` FROM `*PREFIX*musicnc_albums` `AB` JOIN `*PREFIX*musicnc_tracks` `AT` ON `AB`.`id` = `AT`.`album_id` WHERE  `AB`.`artist_id` = ? AND `AB`.`user_id` = ?';
    ...
    private function getTracksDetails($category, $categoryId)
    {
        ...
        $SQL_from = ' FROM `*PREFIX*musicnc_tracks` `AT`
        ...
        if ($category === 'Artist') {
            ...
            $SQL = $SQL_select . $SQL_from .
                'LEFT JOIN `*PREFIX*musicnc_stats` `AS` ON `AT`.`id` = `AS`.`track_id`
            ...
            $SQL = $SQL_select . ' , `AP`.`sortorder`' .
                'FROM `*PREFIX*musicnc_playlist_tracks` `AP` 
					LEFT JOIN `*PREFIX*musicnc_tracks` `AT` ON `AP`.`track_id` = `AT`.`id`
					LEFT JOIN `*PREFIX*musicnc_artists` `AA` ON `AT`.`artist_id` = `AA`.`id`
					LEFT JOIN `*PREFIX*musicnc_albums` `AB` ON `AT`.`album_id` = `AB`.`id`
