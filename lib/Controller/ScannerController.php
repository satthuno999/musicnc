...
class ScannerController extends Controller
{
    ...
    {
        $stmt = $this->db->prepare('SELECT COUNT(`id`) AS `TRACKCOUNT`  FROM `*PREFIX*musicnc_tracks` WHERE `user_id` = ? ');
    ...
    private function getAudioObjects(OutputInterface $output = null)
    {
        ...
        $audios = array_merge($audios_mp3, $audios_m4a, $audios_ogg, $audios_wav, $audios_flac, $audios_aif, $audios_aac, $video_mp4);
    ...
    private function getStreamObjects(OutputInterface $output = null)
    {
        ...
        // get all fileids which are already in the Audio Player Database
        $stmt = $this->db->prepare('SELECT `file_id` FROM `*PREFIX*musicnc_streams` WHERE `user_id` = ? ');
        $stmt->execute(array($this->userId));
        $results = $stmt->fetchAll();
        $resultExisting = array_column($results, 'file_id');
