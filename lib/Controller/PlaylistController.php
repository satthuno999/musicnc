...
class PlaylistController extends Controller
{
    ...
    {
        $stmt = $this->db->prepare('SELECT `id` FROM `*PREFIX*musicnc_playlists` WHERE `user_id` = ? AND `name` = ?');
        $stmt->execute(array($this->userId, $sName));
        $row = $stmt->fetch();
        if ($row) {
            $result = ['msg' => 'exist', 'id' => $row['id']];
        } else {
            $stmt = $this->db->prepare('INSERT INTO `*PREFIX*musicnc_playlists` (`user_id`,`name`) VALUES(?,?)');
            $stmt->execute(array($this->userId, $sName));
            $insertid = $this->db->lastInsertId('*PREFIX*musicnc_playlists');
    ...
    {
        $stmt = $this->db->prepare('UPDATE `*PREFIX*musicnc_playlists` SET `name` = ? WHERE `user_id`= ? AND `id`= ?');
    ...
    {
        $stmt = $this->db->prepare('SELECT COUNT(*) AS tracks FROM `*PREFIX*musicnc_playlist_tracks` WHERE `playlist_id` = ? AND `track_id` = ?');
        $stmt->execute(array($playlistid, $songid));
        $row = $stmt->fetch();
        if ($row['tracks']) {
            return false;
        } else {
            $stmt = $this->db->prepare('INSERT INTO `*PREFIX*musicnc_playlist_tracks` (`playlist_id`,`track_id`,`sortorder`) VALUES(?,?,?)');
    ...
    public function sortPlaylist($playlistid, $songids)
    {
        ...
        foreach ($iTrackIds as $trackId) {
            $stmt = $this->db->prepare('UPDATE `*PREFIX*musicnc_playlist_tracks` SET `sortorder` = ? WHERE `playlist_id` = ? AND `track_id` = ?');
    ...
    {
        try {
            $sql = 'DELETE FROM `*PREFIX*musicnc_playlist_tracks` '
    ...
    {
        try {
            $sql = 'DELETE FROM `*PREFIX*musicnc_playlists` '
