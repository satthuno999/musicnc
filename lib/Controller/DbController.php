...
class DbController extends Controller
{
    ...
    public function loadArtistsToAlbum($iAlbumId, $ARtistID)
    {
        ...
        if ((int)$ARtistID !== 0) {
            $stmt = $this->db->prepare('SELECT `name`  FROM `*PREFIX*musicnc_artists` WHERE  `id` = ?');
            $stmt->execute(array($ARtistID));
            $row = $stmt->fetch();
            return $row['name'];
        } else {
            $stmt = $this->db->prepare('SELECT DISTINCT(`artist_id`) FROM `*PREFIX*musicnc_tracks` WHERE  `album_id` = ?');
    ...
    {
        $searchresult = array();
        $SQL = "SELECT `id`,`name` FROM `*PREFIX*musicnc_albums` WHERE (LOWER(`name`) LIKE LOWER(?)) AND `user_id` = ?";
        ...
        $SQL = "SELECT `AA`.`id`, `AA`.`name` 
                FROM `*PREFIX*musicnc_artists` `AA`
                JOIN `*PREFIX*musicnc_tracks` `AT`
        ...
        $SQL = "SELECT `album_id`, `title` FROM `*PREFIX*musicnc_tracks` WHERE (LOWER(`title`) LIKE LOWER(?)) AND `user_id` = ?";
        $stmt = $this->db->prepare($SQL);
        $stmt->execute(array('%' . addslashes($searchquery) . '%', $this->userId));
        $results = $stmt->fetchAll();
        if (!is_null($results)) {
    ...
    public function resetMediaLibrary($userId = null, $output = null, $hook = null)
    {
        ...
        $stmt = $this->db->prepare('DELETE FROM `*PREFIX*musicnc_tracks` WHERE `user_id` = ?');
        $stmt->execute(array($this->userId));

        $stmt = $this->db->prepare('DELETE FROM `*PREFIX*musicnc_artists` WHERE `user_id` = ?');
        $stmt->execute(array($this->userId));

        $stmt = $this->db->prepare('DELETE FROM `*PREFIX*musicnc_genre` WHERE `user_id` = ?');
        $stmt->execute(array($this->userId));

        $stmt = $this->db->prepare('DELETE FROM `*PREFIX*musicnc_albums` WHERE `user_id` = ?');
        $stmt->execute(array($this->userId));

        $stmt = $this->db->prepare('SELECT `id` FROM `*PREFIX*musicnc_playlists` WHERE `user_id` = ?');

        $stmt->execute(array($this->userId));
        $results = $stmt->fetchAll();
        if (!is_null($results)) {
            foreach ($results as $row) {
                $stmt = $this->db->prepare('DELETE FROM `*PREFIX*musicnc_playlist_tracks` WHERE `playlist_id` = ?');
                $stmt->execute(array($row['id']));
            }
        }

        $stmt = $this->db->prepare('DELETE FROM `*PREFIX*musicnc_playlists` WHERE `user_id` = ?');
        $stmt->execute(array($this->userId));

        $stmt = $this->db->prepare('DELETE FROM `*PREFIX*musicnc_stats` WHERE `user_id` = ?');
        $stmt->execute(array($this->userId));

        $stmt = $this->db->prepare('DELETE FROM `*PREFIX*musicnc_streams` WHERE `user_id` = ?');
        $stmt->execute(array($this->userId));
    ...
    public function deleteFromDB($file_id, $userId = null)
    {
        ...
        if (isset($row['id'])) {
            ...
            $stmt = $this->db->prepare('SELECT COUNT(`album_id`) AS `ALBUMCOUNT`  FROM `*PREFIX*musicnc_tracks` WHERE `album_id` = ? ');
            $stmt->execute(array($AlbumId));
            $row = $stmt->fetch();
            if ((int)$row['ALBUMCOUNT'] === 1) {
                $stmt = $this->db->prepare('DELETE FROM `*PREFIX*musicnc_albums` WHERE `id` = ? AND `user_id` = ?');
                $stmt->execute(array($AlbumId, $this->userId));
            }

            $stmt = $this->db->prepare('DELETE FROM `*PREFIX*musicnc_tracks` WHERE  `file_id` = ? AND `user_id` = ?');
            $stmt->execute(array($file_id, $this->userId));

            $stmt = $this->db->prepare('DELETE FROM `*PREFIX*musicnc_streams` WHERE  `file_id` = ? AND `user_id` = ?');
            $stmt->execute(array($file_id, $this->userId));

            $stmt = $this->db->prepare('SELECT `playlist_id` FROM `*PREFIX*musicnc_playlist_tracks` WHERE `track_id` = ?');
            ...
            if ($row['playlist_id']) {
                ...
                $stmt = $this->db->prepare('SELECT COUNT(`playlist_id`) AS `PLAYLISTCOUNT` FROM `*PREFIX*musicnc_playlist_tracks` WHERE `playlist_id` = ? ');
                $stmt->execute(array($PlaylistId));
                $row = $stmt->fetch();
                if ((int)$row['PLAYLISTCOUNT'] === 1) {
                    $stmt = $this->db->prepare('DELETE FROM `*PREFIX*musicnc_playlists` WHERE `id` = ? AND `user_id` = ?');
                    $stmt->execute(array($PlaylistId, $this->userId));
                }
            }
            $stmt = $this->db->prepare('DELETE FROM `*PREFIX*musicnc_playlist_tracks` WHERE  `track_id` = ?');
            $stmt->execute(array($TrackId));
        }
    ...
    {
        $stmt = $this->db->prepare('UPDATE `*PREFIX*musicnc_albums` SET `cover`= ?, `bgcolor`= ? WHERE `id` = ? AND `user_id` = ?');
    ...
    public function writeAlbumToDB($userId, $sAlbum, $sYear, $iArtistId, $parentId)
    {
        ...
        $stmt = $this->db->prepare('SELECT `id`, `artist_id` FROM `*PREFIX*musicnc_albums` WHERE `user_id` = ? AND `name` = ? AND `folder_id` = ?');
        ...
        if ($row) {
            if ((int)$row['artist_id'] !== (int)$iArtistId) {
                ...
                $stmt = $this->db->prepare('UPDATE `*PREFIX*musicnc_albums` SET `artist_id`= ? WHERE `id` = ? AND `user_id` = ?');
                $stmt->execute(array($various_id, $row['id'], $userId));
            }
            $insertid = $row['id'];
        } else {
            $stmt = $this->db->prepare('INSERT INTO `*PREFIX*musicnc_albums` (`user_id`,`name`,`folder_id`) VALUES(?,?,?)');
            $stmt->execute(array($userId, $sAlbum, $parentId));
            $insertid = $this->db->lastInsertId('*PREFIX*musicnc_albums');
            if ($iArtistId) {
                $stmt = $this->db->prepare('UPDATE `*PREFIX*musicnc_albums` SET `year`= ?, `artist_id`= ? WHERE `id` = ? AND `user_id` = ?');
                $stmt->execute(array((int)$sYear, $iArtistId, $insertid, $userId));
            } else {
                $stmt = $this->db->prepare('UPDATE `*PREFIX*musicnc_albums` SET `year`= ? WHERE `id` = ? AND `user_id` = ?');
    ...
    public function writeArtistToDB($userId, $sArtist)
    {
        ...
        $stmt = $this->db->prepare('SELECT `id` FROM `*PREFIX*musicnc_artists` WHERE `user_id` = ? AND `name` = ?');
        $stmt->execute(array($userId, $sArtist));
        $row = $stmt->fetch();
        if ($row) {
            return $row['id'];
        } else {
            $stmt = $this->db->prepare('INSERT INTO `*PREFIX*musicnc_artists` (`user_id`,`name`) VALUES(?,?)');
            $stmt->execute(array($userId, $sArtist));
            return $this->db->lastInsertId('*PREFIX*musicnc_artists');
        }
    }
    ...
    public function writeGenreToDB($userId, $sGenre)
    {
        ...
        $stmt = $this->db->prepare('SELECT `id` FROM `*PREFIX*musicnc_genre` WHERE `user_id` = ? AND `name` = ?');
        $stmt->execute(array($userId, $sGenre));
        $row = $stmt->fetch();
        if ($row) {
            return $row['id'];
        } else {
            $stmt = $this->db->prepare('INSERT INTO `*PREFIX*musicnc_genre` (`user_id`,`name`) VALUES(?,?)');
            $stmt->execute(array($userId, $sGenre));
            return $this->db->lastInsertId('*PREFIX*musicnc_genre');
        }
    }
    ...
    public function writeTrackToDB($userId, $aTrack)
    {
        $dublicate = 0;
        $insertid = 0;
        $SQL = 'SELECT `id` FROM `*PREFIX*musicnc_tracks` WHERE `user_id`= ? AND `title`= ? AND `number`= ? 
        ...
        if (isset($row['id'])) {
            ...
            $stmt = $this->db->prepare('INSERT INTO `*PREFIX*musicnc_tracks` (`user_id`,`title`,`number`,`artist_id`,`album_id`,`length`,`file_id`,`bitrate`,`mimetype`,`genre_id`,`year`,`folder_id`,`disc`,`composer`,`subtitle`,`isrc`,`copyright`) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)');
            $stmt->execute(array($userId,
                $aTrack['title'],
                $aTrack['number'],
                $aTrack['artist_id'],
            ...
            $insertid = $this->db->lastInsertId('*PREFIX*musicnc_tracks');
        }
        $return = [
            'id' => $insertid,
            'state' => true,
    ...
    public function getTrackInfo($trackId = null, $fileId = null)
    {
        $SQL = "SELECT `AT`.`title` AS `Title`,
                      ...
                      `AT`.`id`
						...
						LEFT JOIN `*PREFIX*musicnc_artists` `AA` ON `AT`.`artist_id` = `AA`.`id`
						LEFT JOIN `*PREFIX*musicnc_genre` `AG` ON `AT`.`genre_id` = `AG`.`id`
						LEFT JOIN `*PREFIX*musicnc_albums` `AB` ON `AT`.`album_id` = `AB`.`id`";
    ...
    {
        $SQL = "SELECT `file_id` FROM `*PREFIX*musicnc_tracks` WHERE  `user_id` = ? AND `id` = ?";
        $stmt = $this->db->prepare($SQL);
    ...
    {
        $SQL = 'UPDATE `*PREFIX*musicnc_tracks` SET `' . $editKey . '` = ? WHERE `user_id` = ? AND `id` = ?';
        $stmt = $this->db->prepare($SQL);
    ...
    {
        $stmt = $this->db->prepare('SELECT `id` FROM `*PREFIX*musicnc_streams` WHERE `user_id` = ? AND `file_id` = ? ');
        ...
        if (isset($row['id'])) {
            ...
            $stmt = $this->db->prepare('INSERT INTO `*PREFIX*musicnc_streams` (`user_id`,`title`,`file_id`,`mimetype`) VALUES(?,?,?,?)');
            $stmt->execute(array($userId,
                $aStream['title'],
                $aStream['file_id'],
                $aStream['mimetype'],
            ));
            $insertid = $this->db->lastInsertId('*PREFIX*musicnc_streams');
        }
        $return = [
            'id' => $insertid,
            'state' => true,
    ...
    public function getPlaylistsForTrack($userId, $trackId)
    {
        ...
        $SQL = "SELECT  `AP`.`playlist_id` , `AN`.`name`, LOWER(`AN`.`name`) AS `lower`
						FROM `*PREFIX*musicnc_playlist_tracks` `AP`
						LEFT JOIN `*PREFIX*musicnc_playlists` `AN` 
