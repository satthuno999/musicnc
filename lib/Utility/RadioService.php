	...
	public function resolveStreamUrl(string $url) : array {
		...
		if ($isPls || $isM3u) {
			...
			if ($status_code != 200) {
				...
				if ($isHls) {
					$resolvedUrl = $this->urlGenerator->linkToRoute('musicnc.radioApi.hlsManifest', ['url' => \rawurlencode($url)]);
				} else {
		...
		if ($result['status_code'] == 200) {
			...
			while ($line = \fgets($fp)) {
				...
				if (!empty($line) && !Util::startsWith($line, '#')) {
					...
					$line = $this->urlGenerator->linkToRoute('musicnc.radioApi.hlsSegment', ['url' => \rawurlencode($segUrl)]);
				}
				$content .= $line . "\n";
			}
