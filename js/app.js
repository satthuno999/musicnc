...
OCA.musicnc.Core = {
    ...
    init: function () {
        ...
        if (decodeURI(location.hash).length > 1) {
            ...
            OCA.musicnc.Backend.getUserValue('category', OCA.musicnc.Core.processCategoryFromPreset);
        }
        ...
        OCA.musicnc.Core.canPlayMimeType.push('audio/mpegurl', 'audio/x-scpls', 'application/xspf+xml');
    },
    ...
    processCategoryFromPreset: function () {
        if (OCA.musicnc.Core.CategorySelectors[0] === 'Albums' || OCA.musicnc.Core.CategorySelectors[0] == null) {
            OCA.musicnc.Core.CategorySelectors[0] = 'Title';
            OCA.musicnc.Core.CategorySelectors[1] = '0';
        }
        document.getElementById('category_selector').value = OCA.musicnc.Core.CategorySelectors[0];
    ...
    selectCategoryItemFromPreset: function () {
        if (OCA.musicnc.Core.CategorySelectors[1]) {
            let activeItem = document.querySelector('#myCategory li[data-id="' + OCA.musicnc.Core.CategorySelectors[1] + '"]');
            activeItem.classList.add('active');
            activeItem.scrollIntoView({behavior: 'smooth', block: 'center',});

            OCA.musicnc.Category.handleCategoryClicked(null, function () {                        // select the last played title
                if (OCA.musicnc.Core.CategorySelectors[2]) {
                    let item = $('#individual-playlist li[data-trackid="' + OCA.musicnc.Core.CategorySelectors[2] + '"]');
                    //item.find('.icon').hide();
                    //item.find('.ioc').removeClass('ioc-volume-up').addClass('ioc-volume-off').show();
                    document.querySelector('#individual-playlist li[data-trackid="' + OCA.musicnc.Core.CategorySelectors[2] + '"]').scrollIntoView({
                        behavior: 'smooth',
                        block: 'center',
                    });
                    if (OCA.musicnc.Core.CategorySelectors[3]) {
                        // if the title was previously played, the last position will be set
                        OCA.musicnc.Player.trackStartPosition = OCA.musicnc.Core.CategorySelectors[3];
                    }
                }
            });
        }
...
OCA.musicnc.Cover = {
    ...
    buildCoverRow: function (aAlbums) {
        ...
        for (let album of aAlbums) {
            ...
            divPlayImage.addEventListener('click', OCA.musicnc.Cover.handleCoverClicked);
    ...
    buildSongContainer: function (eventTarget) {
        ...
        OCA.musicnc.Category.getTracks(null, 'Album', AlbumId, true, albumDirectPlay);
...
OCA.musicnc.Category = {
        ...
        $.ajax({
            ...
            success: function (jsondata) {
                if (jsondata.status === 'success') {
                    ...
                    categoryList.addEventListener('click', OCA.musicnc.Category.handleCategoryClicked);
    ...
    getTracks: function (callback, category, categoryItem, covers, albumDirectPlay) {
        ...
        if (category !== 'Title') {
            document.getElementById('individual-playlist-info').innerHTML = t('musicnc', 'Selected') + ' ' + category + ': ' + category_title;
        } else {
            document.getElementById('individual-playlist-info').innerHTML = t('musicnc', 'Selected') + ': ' + category_title;
        }
    },
...
OCA.musicnc.UI = {
    ...
    buildTrackRow: function (elem, covers) {
        ...
        if (elem['mim'] === 'audio/mpegurl' || elem['mim'] === 'audio/x-scpls' || elem['mim'] === 'application/xspf+xml') {
            ...
            streamUrl.setAttribute('href', OCA.musicnc.UI.getAudiostreamUrl + elem.id);
        }
    ...
    addTitleClickEvents: function (callback) {
        ...
        if ((category[0] === 'Playlist' && category[1].toString()[0] !== 'X' && category[1] !== '')) {
            for (let track of playlist) {
                track.addEventListener("dragover", OCA.musicnc.Playlists.dragover_row_handler);
            }
        }
    ...
    indicateCurrentPlayingTrack: function () {
        if (document.getElementById('playlist-container').dataset.playlist === OCA.musicnc.Player.currentPlaylist) {
            ...
            document.getElementById('nowPlayingText').innerHTML = iocIcon[OCA.musicnc.Player.currentTrackIndex].parentElement.parentElement.dataset.title;
            document.querySelectorAll('.albumwrapper li')[OCA.musicnc.Player.currentTrackIndex].classList.add('isActive');
            document.querySelectorAll('.albumwrapper li')[OCA.musicnc.Player.currentTrackIndex].scrollIntoView(
                {
                    behavior: 'smooth',
                    block: 'center',
                });
        ...
        if (currentTrack) {
            ...
            document.title = currentTrack.dataset.title + ' (' + currentTrack.dataset.artist + ') @ ' + OCA.musicnc.Core.initialDocumentTitle;
        }

        // update sidebar information
        if (document.getElementById('app-sidebar').dataset.trackid !== '') {
            OCA.musicnc.Sidebar.showSidebar(undefined, OCA.musicnc.Player.currentTrackId);
        }
    },
    ...
    handleTitleClicked: function (coverUrl, playlist, element) {
        ...
        if (activeLi.classList.contains('isActive')) {
            ...
            if (document.getElementById('playlist-container').dataset.playlist !== OCA.musicnc.Player.currentPlaylist) {
                ...
                OCA.musicnc.Player.currentPlaylist = document.getElementById('playlist-container').dataset.playlist;
            }
            let k = 0, e = activeLi;
            ...
            if (parseInt(OCA.musicnc.Core.CategorySelectors[2]) !== parseInt(activeLi.dataset.trackid)) {
    ...
    showInitScreen: function (mode) {
        ...
        if (mode === 'smart') {
            OCA.musicnc.UI.EmptyContainer.innerHTML = '<span class="no-songs-found">' + t('musicnc', 'Welcome to') + ' ' + t('musicnc', 'Audio Player') + '</span>';
        } else if (mode === 'playlist') {
            OCA.musicnc.UI.EmptyContainer.innerHTML = '<span class="no-songs-found">' + t('musicnc', 'Add new tracks to playlist by drag and drop') + '</span>';
        } else {
    ...
    sortPlaylist: function (evt) {
        ...
        if (document.getElementById('playlist-container').dataset.playlist === OCA.musicnc.Player.currentPlaylist) {
            ...
            let e = document.querySelectorAll('[src="' + OCA.musicnc.Player.html5Audio.src + '"]')[1];
            if (e) {
                let k = 0;
                while (e = e.previousSibling) {
                    ++k;
...
OCA.musicnc.Backend = {
    ...
    setStatistics: function () {
        ...
        if (track_id) {
            ...
            OCA.musicnc.Backend.setUserValue('category', OCA.musicnc.Core.CategorySelectors[0] + '-' + OCA.musicnc.Core.CategorySelectors[1] + '-' + track_id);
        }
...
OCA.musicnc.Playlists = {
    ...
    renamePlaylist: function (evt) {
        ...
        let boundGenerateRenameRequest = OCA.musicnc.Playlists.generateRenameRequest;
            ...
            function (e) {
                if (e) {
                    $.post(OC.generateUrl('apps/audioplayer/removeplaylist'), {
                        ...
                        if (jsondata.status === 'success') {
                            OCA.musicnc.Category.load();
                            OCP.Toast.success(t('musicnc', 'Playlist successfully deleted!'));
                        }
                    });
                }
            },
    ...
    drop_handler: function (ev) {
        ev.preventDefault();
        OCA.musicnc.Playlists.addSongToPlaylist(this.dataset.id, ev.dataTransfer.getData("id"));
    ...
    dragover_row_handler: function (ev) {
        if (OCA.musicnc.Playlists.isBefore(OCA.musicnc.Core.drag, ev.target.parentNode))
            ev.target.parentNode.parentNode.insertBefore(OCA.musicnc.Core.drag, ev.target.parentNode);
        else
            ev.target.parentNode.parentNode.insertBefore(OCA.musicnc.Core.drag, ev.target.parentNode.nextSibling);
    },
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    OCA.musicnc.UI.resizePlaylist = _.debounce(OCA.musicnc.UI.resizePlaylist, 250);
    document.getElementById('app-content').addEventListener('appresized', OCA.musicnc.UI.resizePlaylist);
    document.getElementById('view-toggle').addEventListener('click', OCA.musicnc.UI.handleViewToggleClicked);
    ...
    document.getElementById('category_selector').addEventListener('change', function () {
        document.getElementById('newPlaylist').classList.add('ap_hidden');
        OCA.musicnc.Core.CategorySelectors[0] = document.getElementById('category_selector').value;
    ...
    document.querySelector('.header-title').addEventListener('click', OCA.musicnc.UI.sortPlaylist);
    document.querySelector('.header-artist').addEventListener('click', OCA.musicnc.UI.sortPlaylist);
    document.querySelector('.header-album').addEventListener('click', OCA.musicnc.UI.sortPlaylist);
