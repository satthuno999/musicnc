...
angular.module('Musicnc').controller('PlaylistDetailsController', [
	...
	function ($scope, $timeout, Restangular, libraryService) {
		...
		$scope.$watch('contentId', function(playlistId) {
			if (!$scope.playlist || playlistId != $scope.playlist.id) {
				...
				$scope.createdDate = OCA.musicnc.Utils.formatDateTime($scope.playlist.created);
				$scope.updatedDate = OCA.musicnc.Utils.formatDateTime($scope.playlist.updated);
		...
		$scope.$watch('playlist.updated', function(updated) {
			$scope.updatedDate = OCA.musicnc.Utils.formatDateTime(updated);
		});
