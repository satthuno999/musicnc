...
angular.module('Musicnc').controller('PlayerController', [
...
function ($scope, $rootScope, playlistService, Audio, gettextCatalog, Restangular, $timeout, $q, $document, $location) {
	...
	if ($location.search().shuffle !== undefined) {
		$scope.shuffle = OCA.musicnc.Utils.parseBoolean($location.search().shuffle);
	}
	if ($location.search().repeat !== undefined) {
		var val = String($location.search().repeat).toLowerCase();
		if (val !== 'one') {
			val = OCA.musicnc.Utils.parseBoolean(val).toString();
		}
		$scope.repeat = val;
	}
	...
	function getPlayableFileUrl(track) {
		for (var mimeType in track.files) {
			if ($scope.player.canPlayMime(mimeType)) {
				return {
					'mime': mimeType,
					'url': OC.filePath('Musicnc', '', 'index.php') + '/api/file/' + track.files[mimeType] + '/download'
				};
			}
		}
	...
	if (!keyboardShortcutsDisabled()) {
		$document.bind('keydown', function(e) {
			if (!OCA.musicnc.Utils.isTextEntryElement(e.target)) {
				var func = null, args = [];
				switch (e.code) {
					case 'Space':
					case 'KeyK':
