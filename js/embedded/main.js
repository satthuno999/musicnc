...
function initEmbeddedPlayer() {
	...
	var mPlayer = new OCA.musicnc.EmbeddedPlayer(onClose, onNext, onPrev, onMenuOpen, onShowList, onImportList, onImportRadio);
	var mPlaylist = new OCA.musicnc.Playlist();
	...
	function onClose() {
		...
		mPlaylist.reset();
		if (OCA.musicnc.playlistTabView) {
			OCA.musicnc.playlistTabView.setCurrentTrack(null, null);
		}
	}
	...
	function onMenuOpen($menu) {
		...
		if (viewingCurrentFileFolder()) {
			...
			$showItem.attr('title', t('Musicnc', 'The option is available only while the parent folder of the playlist file is shown'));
		}
		...
		if (inLibraryFilesCount === 0) {
			...
			$importListItem.attr('title', t('Musicnc', 'None of the playlist files are within your music library'));
		} else {
			$importListItem.removeClass('disabled');
			if (outLibraryFilesCount > 0) {
				$importListItem.attr('title',
						t('Musicnc', '{inCount} out of {totalCount} files are within your music library and can be imported',
	...
	function onShowList() {
		...
		mFileList.showDetailsView(mCurrentFile.name, OCA.musicnc.playlistTabView.id);
	}

	function onImportList() {
		doImportFromFile(OCA.musicnc.playlistFileService.importPlaylist);
	}

	function onImportRadio() {
		doImportFromFile(OCA.musicnc.playlistFileService.importRadio);
	}
	...
	function jumpToPlaylistFile(file) {
		if (!file) {
			...
			if (OCA.musicnc.playlistTabView) {
				...
				OCA.musicnc.playlistTabView.setCurrentTrack(mCurrentFile.id, mPlaylist.currentIndex());
			}
		}
	}
	...
	function register() {
		...
		if (typeof OCA.Files !== 'undefined') {
			OCA.musicnc.initPlaylistTabView(mPlaylistMimes);
	...
	function connectPlaylistTabViewEvents() {
		if (OCA.musicnc.playlistTabView) {
			OCA.musicnc.playlistTabView.on('playlistItemClick', function(playlistId, playlistName, itemIdx) {
			...
			OCA.musicnc.playlistTabView.on('rendered', function() {
	...
	function openPlaylistFile(onReadyCallback /*optional*/) {
		...
		var onPlaylistLoaded = function(data) {
			if (data.files.length > 0) {
				...
				mCurrentFile = null;
				OC.Notification.showTemporary(t('Musicnc', 'No files from the playlist could be found'));
			}
			if (data.invalid_paths.length > 0) {
				var note = t('Musicnc', 'The playlist contained {count} invalid path(s).',
				...
				if (!mShareToken) {
					...
					note += ' ' +  t('Musicnc', 'See the playlist file details.');
				}
		...
		var onError = function() {
			mCurrentFile = null;
			OC.Notification.showTemporary(t('Musicnc', 'Error reading playlist file'));
			mFileList.showFileBusyState($file, false);
		};
		OCA.musicnc.playlistFileService.readFile(mCurrentFile.id, onPlaylistLoaded, onError, mShareToken);
	}
