...
OCA.musicnc.EmbeddedPlayer = function(onClose, onNext, onPrev, onMenuOpen, onShowList, onImportList, onImportRadio) {

	var player = new OCA.musicnc.PlayerWrapper();
	...
	function createPlaylistArea() {
		...
		if (typeof OCA.musicnc.PlaylistTabView != 'undefined') {
	...
	function createPopupMenu() {
		...
		ul.append(createMenuItem('playlist-menu-import-radio', 'icon-radio-nav svg', t('Musicnc', 'Import radio to Music'), onImportRadio));
		ul.append(createMenuItem('playlist-menu-import', 'icon-music-dark svg', t('Musicnc', 'Import list to Music'), onImportList));
		ul.append(createMenuItem('playlist-menu-show', 'icon-menu', t('Musicnc', 'Show playlist'), onShowList));
	...
	function createProgressInfo() {
		const fmt = OCA.musicnc.Utils.formatPlayTime; // give a shorthand name for the utility function
		...
		var loadingText = $(document.createElement('span')).attr('class', 'progress-text').text(t('Musicnc', 'Loading…')).hide();
	...
	function createUi() {
		...
		var getViewWidth = function() {
			...
			// already subtracted.
			if (OCA.musicnc.Utils.getScrollContainer()[0] === parentContainer[0]) {
		...
		OCA.musicnc.DarkThemeLegacySupport.applyOnElement(musicControls[0]);
	...
	function updateMusicAppLink(data, fileId) {
		if (data.in_library) {
			...
			musicAppLinkElements()
				...
				.attr('title', t('Musicnc', 'Continue playing in Music'));
		}
		else {
			musicAppLinkElements().attr('title', t('Musicnc', '(file is not within your music collection folder)'));
		}
	}
	...
	this.show = function(playlistName /*optional*/) {
		...
		if (playlistName) {
			...
			playlistText.text(OCA.musicnc.Utils.dropFileExtension(playlistName));
		} else {
	...
	this.playFile = function(url, mime, fileId, fileName, /*optional*/ shareToken) {
		...
		var fallbackTitle = OCA.musicnc.Utils.titleFromFilename(fileName);
		// Set placeholders for track info fields, proper data is filled once received
		updateMetadata({
			title: t('Musicnc', 'Loading…'),
		...
		changePlayingUrl(function() {
			resolveExtUrl(url, function(resolved) {
				if (shareToken && resolved.hls) {
					OC.Notification.showTemporary(t('Musicnc', 'Stream type not supported on shared file'));
				} else {
