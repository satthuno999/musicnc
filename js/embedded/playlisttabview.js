...
OCA.musicnc.initPlaylistTabView = function(playlistMimes) {
	if (typeof OCA.Files.DetailTabView != 'undefined') {
		OCA.musicnc.PlaylistTabView = OCA.Files.DetailTabView.extend({
			id: 'musicPlaylistTabView',
			className: 'tab musicPlaylistTabView',

			getLabel: function() {
				return t('Musicnc', 'Playlist');
			},
				...
				if (fileInfo) {
					...
					var onPlaylistLoaded = function(data) {
						...
						var titleForFile = function(file) {
							return file.caption || OCA.musicnc.Utils.titleFromFilename(file.name);
						};
						...
						if (data.invalid_paths.length > 0) {
							container.append($(document.createElement('p')).text(t('Musicnc', 'Some files on the playlist were not found') + ':'));
					...
					var onError = function(_error) {
						loadIndicator.hide();
						container.append($(document.createElement('p')).text(t('Musicnc', 'Error reading playlist file')));
					};

					OCA.musicnc.playlistFileService.readFile(fileInfo.id, onPlaylistLoaded, onError);
				}
			},
		...
		_.extend(OCA.musicnc.PlaylistTabView.prototype, OC.Backbone.Events);
		OCA.musicnc.playlistTabView = new OCA.musicnc.PlaylistTabView();

		OC.Plugins.register('OCA.Files.FileList', {
			attach: function(fileList) {
				fileList.registerTabView(OCA.musicnc.playlistTabView);
			}
		});
	}
};
