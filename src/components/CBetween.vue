...
<template>
	<div class="between-container dark:bg-$dark_weak">
		<CLeft v-if="isLeftToggleActive === true"
		...
		<header class="flex justify-between items-center text-gray-500 dark:text-$dark_text">
			<i class="fas fa-bars text-2xl px-2 hidden"
				@click="onLeftToggleClick" />
			<i class="fas fa-arrow-left px-2 cursor-pointer hover:text-black dark:hover:text-white" />
			<i class="fas fa-arrow-right px-4 cursor-pointer hover:text-black dark:hover:text-white" />

			<div class="main-content__search-form flex justify-around px-5 py-2.25 rounded-2xl bg-white dark:bg-$dark_input dark:hover:bg-[#2D4258] w-full cursor-pointer gap-2">
			...
			<i class="fas fa-bars text-2xl px-4 hidden" />
		</header>

		<div class="pt-4 pb-1">
			<p class="main-content__des text-[0.6rem] flex items-center dark:text-$dark_text">
				What's hot<span><img src="/img/fire.png"
					alt="fire_icon"
					width="16px"
					height="16px"></span>
			</p>
		...
		<div class="main-content__trend flex justify-between items-center pb-3.5 dark:text-$dark_text">
			<div class="main-content__trend-title font-semibold">
				Trending
			</div>
			<div class="main-content__trend-more text-xs flex items-center text-gray-500 rounded-3xl py-1 duration-200 cursor-pointer hover:(text-white bg-black px-2)">
				More
				<i class="fas fa-chevron-right" />
			</div>
		</div>
		<div class="main-content__banner grid grid-rows-3 rounded-lg h-40 relative p-5">
			<p class="main-content__banner-artist text-xs">
				Artist
			</p>
			<h4 class="main-content__banner-des">
				On Top Of The World
			</h4>
			<div class="flex justify-between items-center">
				<div class="main-content__banner-btn">
					<button class="play-btn text-white bg-black rounded-3xl py-1 px-3 text-[0.7rem] cursor-pointer mr-2">
						PLAY
					</button>
					<button class="follow-btn text-[0.7rem] rounded-3xl py-1 px-3 border-1 border-solid border-black cursor-pointer">
						FOLLOW
					</button>
				</div>
				<span class="main-content__view text-white text-xs text-right font-light">Monthly listeners
		...
		<div class="my-playlist">
			<div class="playlist-head pt-5 flex justify-between items-center dark:text-$dark_text">
				<h4>Top 100 VN</h4>
				<a class="playlist-more btn-hover hover:(text-white bg-gray-600 px-2) text-gray-500 font-medium rounded-3xl duration-200 cursor-pointer text-xs">Show all</a>
			</div>
			<div class="playlist-list flex justify-between text-xs px-3 py-1 text-gray-400">
				<p class="playlist__number -mr-30">
					#
				</p>
				<p class="playlist__title">
					TITLE
				</p>
				<p class="playlist__artist">
					ARTIST
				</p>
		...
		<div class="overflow-y-scroll h-50 my-1">
			<div v-if="!isLoading">
				<div v-for="(index, i) in main.results.length"
					:key="i"
					...
					<p class="col-span-5">
						{{ main.results[i]["title"] }}
					</p>
					<p class="col-span-5">
						{{ main.results[i]["creator"] }}
					</p>
					<p>{{ main.counter }}</p>
				</div>
			</div>
			<div v-if="isLoading">
				<div v-for="i in 50"
					...
					<p class="bg-gray-300/50 w-10 h-4 rounded-md" />
					<p class="col-span-5 bg-gray-300/50 w-4/5 min-h-full rounded-md" />
					<p class="col-span-5 bg-gray-300/50 w-4/5 min-h-full rounded-md" />
					<p class="bg-gray-300/50 w-4/5 min-h-full rounded-md" />
				</div>
			</div>
		</div>
		...
		<div class="play-song p-3 bg-white rounded-lg dark:bg-$dark_field">
			<div class="play-control grid grid-cols-3 justify-between items-center">
				<div class="play-control__act flex justify-start items-center text-gray-500 dark:text-$dark_text gap-2">
					<div class="dark:bg-$dark_head dark:hover:(bg-$dark_highlight text-$dark_btn) p-1 rounded-md">
						<i v-show="isHeart === false"
							class="far fa-heart"
							@click="onHeart" />
						<i v-show="isHeart === true"
				...
				<div class="play-control__main flex justify-center items-center dark:text-$dark_text gap-2">
					<div class="text-gray-500 dark:text-$dark_text dark:hover:text-$dark_btn text-sm">
						<i class="fas fa-redo-alt play-repeat" />
					</div>
					<div class="dark:text-$dark_btn dark:hover:text-white">
						<i class="fas fa-fast-backward play-backward main-icon" />
					</div>
					<div class="dark:text-$dark_btn dark:hover:text-white">
						<i v-show="isPlaying"
					...
					<div class="dark:text-$dark_btn dark:hover:text-white">
						<i class="fas fa-fast-forward play-forward main-icon" />
					</div>
					<div class="text-gray-500 dark:text-$dark_text dark:hover:text-$dark_btn text-sm">
						<i class="fas fa-random shuffle-song" />
					</div>
				</div>
				<div class="play-control__volume flex justify-end items-center text-gray-500">
					...
					<div class="slidecontainer1 relative">
						<span class="w-[90%] h-0.5 bg-red-500 absolute top-1/2 left-1" />
						<input id="myRange1"
					...
					<div>
						<i class="fas fa-volume-up text-sm" />
					</div>
				</div>
			</div>
			<div class="play-seekbar flex justify-between items-center text-gray-500 text-sm">
				...
				<CInputRange />
				<audio ref="audioPlayer" :src="song" autoplay />
