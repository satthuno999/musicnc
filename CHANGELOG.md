...
- Option `rescan-modified` to the `occ` command `musicnc:scan`
...
- provide settings option to exclude paths from the music library (#762)
  * adding excluded paths takes effect only upon rescan, but the command line tool `occ musicnc:scan --remove-obsolete` can be used for this
- improved performance for the option `--remove-obsolete` of the `occ musicnc:scan` command
...
- add optional argument `--folder=<target>` to `occ musicnc:scan` to scan only the specified folder (#771)
...
- allow rescanning previously scanned tracks with `occ musicnc:scan` by using the option `--rescan`
...
- improved quality of cover images in the Albums view and in the Ampache/Subsonic APIs (#734)
  * default size for the images is now 380px, but this can be altered with `config.php` using the key `musicnc.cover_size`
...
- add option `--remove-obsolete` to the command `occ musicnc:scan` to remove any inaccessible previously scanned files (#567)
...
- add --debug switch to musicnc:scan command to list memory usage of each step
...
- fix some leftovers of the OCA\AppFramework -> OCA\MusicNC\AppFramework change
